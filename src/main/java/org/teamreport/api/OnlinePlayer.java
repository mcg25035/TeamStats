...
public class OnlinePlayer {
    private TeamReport main = (TeamReport) Bukkit.getPluginManager().getPlugin("TeamReport");
    ...
    public static void quitAll(){
        for (Map.Entry i : players.entrySet()){
            ((OnlinePlayer)(i.getValue())).playerQuit();
        }
    }
    ...
    public static void joinAll(){
        for (Player i : Bukkit.getServer().getOnlinePlayers()){
            ...
            if (!org.teamreport.api.OnlinePlayer.players.containsKey(player)){
                org.teamreport.api.OnlinePlayer.playerJoin(player);
            }
        }
    }
